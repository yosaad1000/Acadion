...
class NotificationService:
    ...
    def __init__(self):
        try:
            # TEMPORARY: Use mock data until Supabase client proxy issue is resolved
            logger.info("NotificationService initialized in MOCK mode (Supabase client proxy issue)")
            self.supabase = None  # Will use mock data
            self._connection_healthy = True  # Set to True to enable mock functionality
            self._mock_notifications = []  # Store mock notifications
            self._mock_preferences = []  # Store mock preferences
            
            # Create some sample notifications for testing
            from app.models.notification import NotificationResponse, NotificationType
            from datetime import datetime
            ...
            sample_notifications = [
                ...
                },
                {
                    ...
                    'title': 'System Status',
                    'message': 'Notification service is running in mock mode until Supabase client is fixed.',
                    'data': {},
                    'is_read': False,
                    'created_at': datetime.utcnow(),
                    'updated_at': datetime.utcnow()
            ...
            for notif_data in sample_notifications:
                notification = NotificationResponse(**notif_data)
                self._mock_notifications.append(notification)
                
        except Exception as e:
            logger.error(f"Error initializing mock NotificationService: {e}")
    ...
    async def get_user_notifications(self, user_id: str, limit: int = 50, offset: int = 0) -> List[NotificationResponse]:
        ...
        if self.supabase is None:
            logger.info(f"Returning {len(self._mock_notifications)} mock notifications for user {user_id}")
            # Apply limit and offset to mock data
            start = offset
            end = offset + limit
            return self._mock_notifications[start:end]
        ...
        try:
            ...
            result = await self._execute_with_retry(fetch_operation)
    ...
    async def mark_as_read(self, notification_id: str, user_id: str) -> bool:
        ...
        # MOCK MODE: Mark mock notification as read
        if self.supabase is None:
            for notification in self._mock_notifications:
                if notification.id == notification_id:
                    notification.is_read = True
                    logger.info(f"Mock notification {notification_id} marked as read for user {user_id}")
                    return True
            logger.warning(f"Mock notification {notification_id} not found for user {user_id}")
            return False
    ...
    async def get_unread_count(self, user_id: str) -> int:
        ...
        # MOCK MODE: Count unread mock notifications
        if self.supabase is None:
            unread_count = len([n for n in self._mock_notifications if not n.is_read])
            logger.info(f"Returning mock unread count: {unread_count} for user {user_id}")
            return unread_count
    ...
    async def get_user_preferences(self, user_id: str) -> List[NotificationPreferenceResponse]:
        ...
        # MOCK MODE: Return mock preferences
        if self.supabase is None:
            if not self._mock_preferences:
                # Create default mock preferences
                from app.models.notification import NotificationPreferenceResponse, NotificationType
                from datetime import datetime
                
                for notif_type in NotificationType:
                    pref = NotificationPreferenceResponse(
                        id=f"mock-pref-{notif_type.value}",
                        user_id=user_id,
                        notification_type=notif_type,
                        enabled=True,
                        created_at=datetime.utcnow()
                    )
                    self._mock_preferences.append(pref)
            
            logger.info(f"Returning {len(self._mock_preferences)} mock preferences for user {user_id}")
            return self._mock_preferences
